    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user is not None:
                ...
                return JsonResponse({'errors': {'non_field_errors': ['Email or password is not valid']}}, status=status.HTTP_202_ACCEPTED)
        return JsonResponse(serializer.errors, status=status.HTTP_202_ACCEPTED)
...
class AddressView(APIView):
    ...
    def put(self, request, format=None):
        ...
        serializer = AddressSerializer(
            instance=profile_data.address, data=data)
...
class CallLogView(generics.ListAPIView):
    ...
    def post(self, request, format=None):
        ...
        return JsonResponse({"message": "Call-logs has been successfully added"}, status=status.HTTP_200_OK, safe=False)
    ...
    def post(self, request, format=None):
        for data in request.data:
            ...
            if serializer.is_valid():
                ...
                return JsonResponse(serializer.errors, status=status.HTTP_400_BAD_REQUEST, safe=False)
        return JsonResponse({"message": "Messages has been successful added."}, status=status.HTTP_200_OK, safe=False)
    ...
    def post(self, request, format=None):
        ...
        for user_item in request.data:
            if User.objects.filter(email=user_item['email']).exists():
                ...
                children_user_requests = Children.objects.get(
                    user=user_requested)
